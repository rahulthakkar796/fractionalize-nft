...
describe("Tests for ERC1155 approach", async function () {
  ...
  // function to transfer fractionalized tokens to test accounts to test the batch transfer
  async function transferTokens(account, transferDetails) {
    ...
    expect(balance).to.be.equal(BigNumber.from(transferDetails.amount));
  }

  // function to approve fractionalized tokens to test account to test the batch transfer
  ...
  it("Test batch transfer", async () => {
    ...
    // generate 280 test accounts to process and test batch transfer(can process upto 310 transfers per call)
    ...
    const batchTransfer = await nftContract
      .connect(accounts[10])
