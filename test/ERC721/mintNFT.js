...
describe("Tests for ERC721+ERC20 approach", async function () {
  ...
  async function batchTransfer(from, to, amounts) {
    try {
      ...
      expect(to.length).to.be.equal(
        _to.length,
        "to array length doesn't match"
      );
  ...
  describe("Mint and Fractionalize", () => {
    it("Mint NFT and fractionalize it", async () => {
      const params = {
        ...
        symbol: "FPH",
        totalSupply: ethers.utils.parseEther("3000000"),
      };
  ...
  describe("Batch test(Many-To-Many)", () => {
    it("test batch transfer(many-to-many)", async () => {
      const amount = ethers.utils.parseEther("10000").toString();
  ...
  describe("Batch test(One-To-Many)", () => {
    it("Test batch transfer(one-to-many)", async () => {
      const amount = ethers.utils.parseEther("20000").toString();
